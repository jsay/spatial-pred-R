...
## DROPPING THE UNITS WITHOUT NEIGHBORS
tst.sp <- subset(bst.sp, bst.sp$SMP== "IS")
tst.nb <- subset(bst.nb, bst.sp$SMP== "IS")
ins.sp <- subset(tst.sp, card(tst.nb)!= 0)
ins.nb <- subset(tst.nb, card(tst.nb)!= 0)
tst.sp <- subset(bst.sp, bst.sp$SMP== "OS")
tst.nb <- subset(bst.nb, bst.sp$SMP== "OS")
ots.sp <- subset(tst.sp, card(tst.nb)!= 0)
ots.nb <- subset(tst.nb, card(tst.nb)!= 0)
...
par(mar= c(0, 0, 0, 0)) ; plot(bst.sp, border= FALSE)
plot(ins.sp, col= "grey30", add= TRUE, border= FALSE)
plot(ots.sp, col= "grey70", add= TRUE, border= FALSE)
plot(bst.nb, coordinates(bst.sp), add= T, col= "green", pch= 20, cex= .2)
plot(ins.nb, coordinates(ins.sp), add= T, col= "blue" , pch= 20, cex= .2)
plot(ots.nb, coordinates(ots.sp), add= T, col= "red"  , pch= 20, cex= .2)
legend("topleft", col= c("green", "blue", "red"), lwd= 1.5,
       c("PRD NBH", "IS NBH", "OS NBH"))
legend("right", fill= c("grey30", "grey70"), c("IN-SAMPLE", "OUT-OF-SAMPLE"))
#+end_src
...
[[file:Figures/SpWeigthBoston.pdf]]
   ...
   The issue is now to build the =nb= object with the green
   links. Because of the symmetries, we can do that by elimination.
...
ots.nb <- poly2nb(ots.sp) ; ost.lw <- nb2listw(ots.nb)
...
bst.mt <- data.frame("ID"= rep(row.names(bst.sp), card(bst.nb)),
                     "NBH"= unlist(bst.nb))
ins.mt <- data.frame("ID"= rep(row.names(ins.sp), card(ins.nb)),
                     "NH"= unlist(ins.nb), "IS"= 1)
ots.mt <- data.frame("ID"= rep(row.names(ots.sp), card(ots.nb)),
                     "NH"= unlist(ots.nb), "OS"= 1)
tmpi <- data.frame("NBH"= row.names(ins.sp),
                   "NH"= 1: length(row.names(ins.sp)))
tmpo <- data.frame("NBH"= row.names(ots.sp),
                   "NH"= 1: length(row.names(ots.sp)))

att <- subset(bst.nb, row.names(bst.sp) %in% row.names(tst.sp))

cal
cal <- unique(round(runif(250)* 500)),

ins <- merge(ins.mt, tmpi, by= c("NH"))[, -1]
ots <- merge(ots.mt, tmpo, by= c("NH"))[, -1]


yop <- merge(bst.mt, ins, by= c("ID", "NBH"), all.x= TRUE)
yap <- merge(bst.mt, ots, by= c("ID", "NBH"), all.x= TRUE)


plot(bst.sp[379: 385, ])
text(coordinates(bst.sp), row.names(bst.sp), col ="blue")

text(coordinates(ins.sp), row.names(ins.sp), col ="red")
...
length(unlist(bst.nb))

length(unlist(ins.nb))
length(unlist(ots.nb))

#+end_src

** Subset weight matrix

   To subset a weight matrix symmetrically as it is the case actually,
   we can select only attributes of focal point and target
   point. define Focal and target slots.

#+begin_src R :tangle yes ./spsubset.R
spsubset <- function (x, focal, target= NULL, ...){
    ...
    old.ids <- 1: n
    new.ids <- match(old.ids, which(focal))
    reg.id <- subset.default(attr(x, "region.id"), focal)
    x <- sym.attr.nb(x)
    xattrs <- names(attributes(x))
    z <- subset.default(x, focal)
    nz <- length(z)
    for (i in 1: nz){
...
rmse(semLSP, sem)
semKP2 <- sppred(sem, blup= "KP2", loo= TRUE, power= TRUE)
rmse(semKP2, sem)
...
rmse(sarLSP, sar)
sarKP2 <- sppred(sar, blup= "KP2", loo= TRUE, power= TRUE)
rmse(sarKP2, sar)
...
rmse(sdmLSP, sem)
sdmKP2 <- sppred(sdm, blup= "KP2", loo= TRUE, power= TRUE)
rmse(sdmKP2, sem)
