...
implements among other the predictors from \cite{LPac04, PLes08} and
\cite{KPru07} for a large number of specifications of spatial
autocorrelation from the [[url:http://cran.r-project.org/web/packages/spdep/index.html][=spdep= package]] (\cite{Biva14}). To use this
code, save the file [[./sppred.R][=sppred=]] in your working directory then submit
=source("sppred.R")= to R. Some examples from =boston= data are also
available. The status of this work is actually under construction,
...
- Allow models with transformed outcome variable
#+END_abstract
#+TOC: headlines 1
...
** Spatial econometric models
   ...
   specification of interdependence between observation, typically
   from their spatial proximity.  Consequently, resulting predictions
   not only depend on the variables of the target units (i.e., the
   units for which we want to predict the outcome) but potentially
   involve the entire set of both explanatory and dependent variables
   of sample data observations.  This implies that the sample pattern
   of the outcomes or the residuals contains additional information
   which may be used to modify the regression function so as to reduce
   the prediction variance [[ref:Gold62]["p,(Goldberger 1962)]].
   ...
   notations: see \cite{LPac04, PLes08} \cite{KPru07}, and
   \cite{TALG13}.  A first contribution of this work is to unify the
   ...
   performance on a well-known dataset (=boston= from \cite{Biva14}).
...
u & = \lambda W u+ \varepsilon \;\;\mbox{ with }\;\; 
\varepsilon\sim \mathbf{N}(0, \sigma^2_{\varepsilon}I_N)\nonumber
\end{align}
   ...
   matrix (see \cite{Anse88}). We limit ourselves to a same weight
   ...
   $\sigma_{\varepsilon}$ grouped in $\Theta$ have to be estimated, as
   the vector $\varepsilon$ of innovations. Classically, we assume
   that $\mbox{diag}(W)= 0$ and $|\,\rho\,|$, $|\,\lambda\,|< 1$. The
   ...
   model (SXM, also called Spatial Durban Error Model by
   \cite{LPac08}) with $\rho=0$ and the spatial error model (SEM) can
   be recovered with $\rho=\theta= 0$. All these models can be
   estimated by maximum likelihood through the functions =errorsarlm=,
   =lagsarlm=, and =sacsarlm= of the *R* package =spdep=
   \cite{Biva14}.

   For future reference, note that the general SMC model can be
   reduced in $y= (I-\rho W)^{-1}X\beta +(I-\rho W)^{-1}(I-\lambda
   W)^{-1}\varepsilon$, which is parametrized by a distribution that
   involves all of the model parameters:
...
\end{equation}
   ...
   depends on the sample size, data quality, etc.  Moreover, all the
   ...
   conditioning sets for the sake of clarity.  

   (peut-être mettre du os des ici) With such autocorrelation in the
   stochastic components, it was known since \cite{Gold62} that the
   best linear unbiased estimator of $y$ is not the best linear
   unbiased predictor (BLUP).  Applying the BLUP formula allows us to
   obtain:

\begin{equation}
\mathbf{E}(y\mid \Lambda)= \alpha+ \rho W\mathbf{E}(y \mid \Lambda) +X\beta + WX\theta +\mathbf{E}(u \mid \Lambda) 
\end{equation}

   As we will see below, defining the conditional set $\Lambda$ in
   spatial models is not trivial.  Some inconsistencies in the
   literature reinforce this feeling.

   Previous equation is fairly general, as it uses only the exogeneity
   of the predictors $X$ applied to equation (1).  So its not the true
   BLUP but an approximation, in this framework parameters are
   exogenous. In the bivariate formulation, This is the general BLUP
   framework shared by all the literature.  The minimum mean square
   error predictor (the best) is the conditional mean corresponding to
   the information set.  [normal expression here]. Our distinction
   between stochastic and non stochastic part can be linked to signal
   and trend terminology [[cite:Hain93]].
...
*** The information sets
   ...
   The interest in predicting from spatial econometric models are
   multiple and the code used for this have to be customizable because
   different reasons lead to different predictors. One can cite with
   [[cite:PLes08]]:

   - implement counter-factual predictions to better interpret (direct
     and indirect effects)
   - extrapolate into the future the outcome variable of interest
   - predictors are smoother, so more easily understood as a map
   - Detect general spatial patterns, for the effect of modeling space (trends
     vs signal)
   - fill in the gap in the case of spatially missing observations of
     the outcome variable.

   Trade-off between simplicity and efficiency according to the
   information set used, and it depends on case study so it is
   important for a software to implement numerous.
...
\end{equation}
   ...
   is the default predictor of the function =sppred=. As LSP shown,
   the $X$ from the neighbors are used to predict.
...
*** Leave-one-out predictions
    ...
    In the loo case, it is strange because the information set is
    different for each prediction and there is a contradiction about
    the information sets between two predictions. Without this
    contradiction we are in LSP with partitioned matrix.
...
\end{equation}
   ...
   We can then take an example of custom predictions with the ex post
   forecasting.
...
#+RESULTS:
:      SEM      SXM      SAR      SDM      SAC      SMC    
: [1,] 445.7127 433.3333 435.5886 434.1438 436.3016 435.197
...
#+begin_src R :results output exemple
source("sppredF.R")
system.time(SEMprdKP2 <- sppred(SEM, blup= "KP2", loo= TRUE, tst= 2))
sqrt(mean(I(SEMprdKP2- SEM$y)^2))
source("sppred.R")
system.time(SEMpddKP2 <- sppred(SEM, blup= "KP2", loo= TRUE, tst= 2))
sqrt(mean(I(SEMpddKP2- SEM$y)^2))
...
#+begin_src R :results output exemple
source("sppredF.R")
system.time(SEMprdKP3 <- sppred(SEM, blup= "KP3", loo= TRUE))
sqrt(mean(I(SEMprdKP3- SEM$y)^2))
source("sppred.R")
system.time(SEMpddKP3 <- sppred(SEM, blup= "KP3", loo= TRUE))
sqrt(mean(I(SEMpddKP3- SEM$y)^2))
system.time(SEMpddCTM <- sppred(SEM, ctm= TRUE))
sqrt(mean(I(SEMpddCTM- SEM$y)^2))
...
#+end_src
   ...
   It is OK. Now, we look if this property stay true in the case of
   SAC model

*** For the SAC case

#+begin_src R :results output exemple
Zr <- (diag(dim(WALL)[ 1])- (.33* WALL))
Zl <- (diag(dim(WALL)[ 1])- (.55* WALL))
## METHODE PAR LA MAT COV-VAR
OMG <- solve(Zr) %*% solve(Zl) %*% solve(t(Zl)) %*% solve(t(Zr)) 
## isSymmetric(OMG) TRUE
OMG11 <- OMG[ CAL,  CAL] ; OMG22 <- OMG[!CAL, !CAL]
OMG12 <- OMG[ CAL, !CAL] ; OMG21 <- OMG[!CAL,  CAL]
BLUP1 <- OMG21 %*% solve(OMG11)
BLUP1[1: 10, 1: 5]
## METHODE PAR LA MATRICE DE PRECISION
PHI <- t(Zr) %*% t(Zl) %*% Zl %*% Zr # isSymmetric(PHI) TRUE
PHI11 <- PHI[ CAL,  CAL] ; PHI22 <- PHI[!CAL, !CAL]
PHI12 <- PHI[ CAL, !CAL] ; PHI21 <- PHI[!CAL,  CAL]
BLUP2 <- - solve(PHI22) %*% PHI21
BLUP2[1: 10, 1: 5]
#+end_src
   ...
   It is also OK, and it seems to be a good mean to simplify KP. It
   stay the question about using the computing capacity of =spdep= in
   this framework.

*** For the KP case (SAC)

    First we verify the equivalence in covariance terms.
...
## METHODE PAR LA MAT COV-VAR
OMG <- solve(Zr) %*% solve(Zl) %*% solve(t(Zl)) %*% solve(t(Zr))
OMG <- solve(t(Zl) %*% t(Zr) %*% Zr %*% Zl)
BLUP1 <- OMG[1,-1] %*% solve(OMG[-1, -1])
BLUP1[1: 3]
## METHODE PAR KP3 stricto sensus
NUM  <- (solve(Zl) %*% solve(t(Zl))) %*% solve(t(Zr))
KP3 <- NUM[1, -1] %*% solve(OMG[-1, -1])
KP3[1: 3] 
## On retrouve KP3 par LSP
rr <- (Zr %*% OMG)[1, -1] %*% solve(OMG[-1, -1])
rr <- Zr[1, ] %*% OMG[, -1] %*% solve(OMG[-1, -1])
rr[1: 3]
## Avec la méthode des matrices de prec
PHI <- t(Zr) %*% t(Zl) %*% Zl %*% Zr    
rrr <- - solve(PHI[1, 1]) %*% (Zr[1, ] %*% PHI[,-1])
rrr[1: 3] ## PAS PAREIL, ON PEUT PAS UTILISER PREC AVEC KP3
## METHODE PAR KP2 stricto sensus
(KP2 <- NUM[1, ] %*% WALL[1, ] %*% solve(WALL[1, ] %*% OMG %*% WALL[1, ]))
## On retrouve KP2 par LSP
ss <- (Zr %*% OMG)[1, ] %*% WALL[1, ] %*% solve(WALL[1, ] %*% OMG %*% WALL[1, ])
ss
uu <- Zr[1, ] %*% OMG %*% WALL[1, ] %*% solve(WALL[1, ] %*% OMG %*% WALL[1, ])
uu
#+end_src

*** KP predictors without loo (SAC)

#+begin_src R :results output exemple
Zr <- (diag(dim(WALL)[ 1])- (.33* WALL))
Zl <- (diag(dim(WALL)[ 1])- (.55* WALL))
## CALCUL DE LA MAT COV-VAR
OMG <- solve(t(Zl) %*% t(Zr) %*% Zr %*% Zl)
## LOOP KP3
blupKP3 <- rep(NA, 299)
system.time(
for(i in 1: 200){
    blupKP3 <- rbind(blupKP3, (Zr %*% OMG)[i, -i] %*% solve(OMG[-i, -i]))
})
head(blupKP3[, 1: 5])
bKP3 <- (Zr %*% OMG)[1: 5, -c(1: 5)] %*% solve(OMG[-c(1: 5), -c(1: 5)])
head(bKP3[, 1: 5])
## LOOP KP2
blupKP2 <- c(NA)
system.time(
...
** Custom predictions
   ...
   It can be called ex-post predictions because explanatory variables
   are known.
...
** Changes relative to =predict.sarlm=
   ...
   - Drop the use of =tarX= and =tary= that can produce a loss of
     rapidity
...
[fn:1] This terminology comes from \cite{LPac08} and
  \cite{BPGo08}. This model is also called spatial autoregressive
  model with autoregressive disturbances, SARAR(1,1), by
  \cite{KPru98}. The notations of spatial lag coefficients also
  depends on authors, we adopt here those of the *R* package =spdep=
  (\cite{Biva14}).
