...
implements in particular the predictors from \cite{LPac04, PLes08} and
...
=source("sppred.R")= to *R*. Some examples on the =boston= data are
...
** Spatial econometric models
   ...
   as to reduce the prediction variance [[cite:Gold62]].
   ...
   notations: see \cite{LPac04, PLes08} and \cite{KPru07}.  A first
   ...
   framework.  A second contribution is to present the *R* function
   =sppred= that implements all the spatial predictors and presents
   some empirical evidence on their relative performance on a
   well-known dataset (=boston=, \cite{Biva14}).
...
\end{equation}
   ...
   approximation, in this framework parameters are exogenous. In the
   bivariate formulation, This is the general BLUP framework shared by
   all the literature.  The minimum mean square error predictor (the
   best) is the conditional mean corresponding to the information set.
   [normal expression here]. In the loo case, it is strange because
   the information set is different for each prediction and there is a
   contradiction about the information sets between two
   predictions. Without this contradiction we are in LSP with
   partitioned matrix. Our distinction between stochastic and non
   stochastic part can be linked to signal and trend terminology
   [[cite:Hain93]].
