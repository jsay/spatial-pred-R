...
** Neighborhood relationships
   ...
   links. Because of the symmetries, we can do that by elimination. We
   have a strong difference with the classical =subset.nb= matrix
   because some absent units have to be referred by present units. So
   we do not change the dimension of the =nb= matrix, we put 0s
   instead.

   To this work about spatial weights, we create our own subset
   functions (see XX).
...
W11 <- nbsubset(bst.nb, bst.sp$SMP== "IS", target= bst.sp$SMP== "IS")
W12 <- nbsubset(bst.nb, bst.sp$SMP== "IS", target= bst.sp$SMP== "OS")
W21 <- nbsubset(bst.nb, bst.sp$SMP== "OS", target= bst.sp$SMP== "IS")
W22 <- nbsubset(bst.nb, bst.sp$SMP== "OS", target= bst.sp$SMP== "OS")
...
plot(W11, coordinates(bst.sp), add= T, col= "green", pch= 20, cex= .2)
plot(W12, coordinates(bst.sp), add= T, col= "blue", pch= 20, cex= .2)
plot(W22, coordinates(bst.sp), add= T, col= "red", pch= 20, cex= .2)
...
#+end_src
   ...
   And the corresponding function for the =listw= objects, be careful
   about symmetry
